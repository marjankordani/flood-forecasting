...
class DeepMix:
    def __init__(self,
        ...
        self.Models = []
        self.sSplit = 1
        self.nConvolution = nConvolution
    ...
    def CreateModel(self):
        ...
        for i in range(self.nModel):
            ...
            O = None
            for i in self.nConvolution:
                O = lay.Conv1D(filters=i,
                               kernel_size=self.sKernel,
                               strides=1,
                               padding='same',
                               activation=act.linear,
                               kernel_regularizer=reg.l1_l2(l1=self.L1, l2=self.L2))({True: I, False: O}[O is None])
                O = lay.LeakyReLU(alpha=self.Alpha)(O)
                O = lay.Dropout(self.rDO)(O)
            for i, j in enumerate(self.nLSTMs):
                O = lay.Bidirectional(lay.LSTM(units=j,
                                               kernel_regularizer=reg.l1_l2(l1=self.L1, l2=self.L2),
                                               return_sequences=(i != len(self.nLSTMs) - 1)))(O)
                O = lay.Dropout(self.rDO)(O)
            O = lay.Flatten()(O)
            O = lay.Attention()([O, O])
...
nConvolution = [32, 64] # Convolution Layers Size
sKernel = 3 # Convolution Layers Kernels Size
Alpha = 0.2 # Leaky-ReLU Layers Alpha
